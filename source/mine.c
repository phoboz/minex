	...
	)
{
	...
	while (mine != 0)
	{
		...
		else if (mine->lo_counter == 3)
		{
			...
			else if (mine->state == MINE_STATE_FIREBALL_IDLE)
			{
				if (++mine->hi_counter == MINE_FIREBALL_IDLE_TRESHOLD)
				{
					mine->hi_counter = 0;
...
//#define DEBUG_DRAW
			...
			else if (mine->state == MINE_STATE_FIREBALL_IDLE || mine->state == MINE_STATE_FIREBALL)
			{
#ifdef DEBUG_DRAW
